...
Spec 6.4. Etapa 1. Sprint 1 con implementación y validación local completas.
Estado: `LOCAL_PASS_DOCKER_PENDING`. No se marca cierre operativo total porque el
build/runtime de Docker no se pudo comprobar. Rama `codex/sprint-1`, sin commits al cierre.
...
Dockerfile/Compose y scripts Windows. `README.md` explica ejecución y límites.
...
- `scripts/check.ps1`: PASS; ruff, format, 30 tests y smoke HTTP con proceso real.
...
Para continuar funcionalidad: definir sprint 2 del universo Nasdaq, verificar fuentes oficiales,
